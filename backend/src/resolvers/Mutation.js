...
const Mutations = {
  ...
  async deleteItem(parent, args, ctx, info) {
    ...
    if (!ownsItem && !hasPermissions) {
      throw new Error("You don't permission to do that");
    }
    // 3. Delete it
  ...
  async addToCart(parent, args, ctx, info) {
    ...
    // 3. Check if that item is already in their card and
  ...
  async removeFromCart(parent, args, ctx, info) {
    ...
    if (cartItem.user.id !== ctx.request.userId) {
      throw new Error("You can't delete items form someone else's cart");
    }
